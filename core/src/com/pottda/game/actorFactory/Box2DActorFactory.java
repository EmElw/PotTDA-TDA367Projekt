    ...
    public AIController buildEnemy(Sprites sprite, Vector2f position, Inventory inventory) {
        ...
        switch (ControllerOptions.AISettings) {
            case ControllerOptions.DUMB_AI:
                aiController = new DumbAIController(model, view);
                break;
            case ControllerOptions.PATHFINDING_AI:
                aiController = new PathfindingAIController(model, view, ControllerOptions.pathfinder);
                break;
        }
        return aiController;
    }
